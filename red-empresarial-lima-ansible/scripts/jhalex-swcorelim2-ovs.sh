#!/usr/bin/env bash
# =============================================================================
# jhalex-swcorelim2-ovs.sh
# Script OVS idempotente para SWCORELIM2 — Red Empresarial Lima (JHALEX)
# =============================================================================
# Propósito:
#   Crear y mantener la configuración Open vSwitch de SWCORELIM2 de forma
#   idempotente.
# =============================================================================
...
echo ""
echo "========================================================================"
echo "  SWCORELIM2 — Configuración Open vSwitch (JHALEX)"
...
echo ""
log_step "PASO 2: Bond bond-pcsc1-sc2 (Core → SWCORELIM1)"
...
echo ""
log_step "PASO 3: Bond bond-pcsc2-sd1 (Core → SWDISTLIM1)"
if ovs-vsctl list-ports br-core 2>/dev/null | grep -q "^bond-pcsc2-sd1$"; then
    log_warn "bond-pcsc2-sd1 ya existe. No se recrea."
else
    BOND_SD1_MEMBERS=()
    for iface in ens38 ens39; do
        if ip link show "$iface" &>/dev/null; then
            ...
            log_warn "${iface} no disponible. Excluido de bond-pcsc2-sd1."
        fi
    done
    if [[ ${#BOND_SD1_MEMBERS[@]} -ge 1 ]]; then
        ovs-vsctl add-bond br-core bond-pcsc2-sd1 "${BOND_SD1_MEMBERS[@]}" bond_mode=balance-slb
        log_ok "bond-pcsc2-sd1 creado con: ${BOND_SD1_MEMBERS[*]}"
    else
        log_warn "Sin miembros para bond-pcsc2-sd1. Bond no creado."
    fi
fi
if ovs-vsctl list-ports br-core 2>/dev/null | grep -q "^bond-pcsc2-sd1$"; then
    ovs-vsctl set port bond-pcsc2-sd1 bond_mode=balance-slb
    log_ok "Modo balance-slb confirmado en bond-pcsc2-sd1."
fi

echo ""
log_step "PASO 4: Bond bond-pcsc2-sd2 (Core → SWDISTLIM2)"
if ovs-vsctl list-ports br-core 2>/dev/null | grep -q "^bond-pcsc2-sd2$"; then
    log_warn "bond-pcsc2-sd2 ya existe. No se recrea."
else
    BOND_SD2_MEMBERS=()
    for iface in ens40 ens41; do
        if ip link show "$iface" &>/dev/null; then
            ...
            log_warn "${iface} no disponible. Excluido de bond-pcsc2-sd2."
        fi
    done
    if [[ ${#BOND_SD2_MEMBERS[@]} -ge 1 ]]; then
        ovs-vsctl add-bond br-core bond-pcsc2-sd2 "${BOND_SD2_MEMBERS[@]}" bond_mode=balance-slb
        log_ok "bond-pcsc2-sd2 creado con: ${BOND_SD2_MEMBERS[*]}"
    else
        log_warn "Sin miembros para bond-pcsc2-sd2. Bond no creado."
    fi
fi
if ovs-vsctl list-ports br-core 2>/dev/null | grep -q "^bond-pcsc2-sd2$"; then
    ovs-vsctl set port bond-pcsc2-sd2 bond_mode=balance-slb
    log_ok "Modo balance-slb confirmado en bond-pcsc2-sd2."
fi
...
ip addr replace 10.255.21.2/30 dev int-core-test 2>/dev/null || \
    ip addr add 10.255.21.2/30 dev int-core-test 2>/dev/null || \
    log_warn "IP 10.255.21.2/30 ya asignada (normal)."
ip link set int-core-test up 2>/dev/null || true
log_ok "int-core-test UP con 10.255.21.2/30"
...
echo "========================================================================"
echo "  SWCORELIM2 — OVS configurado"
