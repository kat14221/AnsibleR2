...
# ============================================================
# SCRIPT DE REFERENCIA — SWACCLIM1 OVS
# NOTA: La copia canónica está en roles/swacclim1/files/
# ============================================================

echo "[JHALEX] Configurando OVS SWACCLIM1..."
...
# Valor 32768 = default OVS; para asegurar no-root usamos 28672 (menor que Dist pero mayor = no preferido)
ovs-vsctl set bridge br-acc other_config:rstp-priority=28672 || true

# --- Bond 1: SWACCLIM1 ↔ SWDISTLIM1 (PCSD1-SA1) ---
# Members: ens34 (primary), ens35
ovs-vsctl add-bond br-acc bond-pcsd1-sa1 ens34 ens35
ovs-vsctl set port bond-pcsd1-sa1 bond_mode=active-backup
ovs-vsctl set port bond-pcsd1-sa1 lacp=off
ovs-vsctl set port bond-pcsd1-sa1 other_config:bond-primary=ens34
ovs-vsctl set port bond-pcsd1-sa1 trunks=10,20,30,40,50,60,70,80,99

# --- Bond 2: SWACCLIM1 ↔ SWDISTLIM2 (PCSA1-SD2) ---
# Members: ens36 (primary), ens37
ovs-vsctl add-bond br-acc bond-pcsa1-sd2 ens36 ens37
ovs-vsctl set port bond-pcsa1-sd2 bond_mode=active-backup
ovs-vsctl set port bond-pcsa1-sd2 lacp=off
ovs-vsctl set port bond-pcsa1-sd2 other_config:bond-primary=ens36
ovs-vsctl set port bond-pcsa1-sd2 trunks=10,20,30,40,50,60,70,80,99

ip link set br-acc up || true

echo "[JHALEX] SWACCLIM1 - OVS configurado correctamente"
echo "[JHALEX] Bridge: br-acc | RSTP: habilitado (nunca root)"
echo "[JHALEX] Bonds:"
echo "[JHALEX]   bond-pcsd1-sa1 (ens34+ens35) → Dist1 | trunk 10,20,30,40,50,60,70,80,99"
echo "[JHALEX]   bond-pcsa1-sd2 (ens36+ens37) → Dist2 | trunk 10,20,30,40,50,60,70,80,99"
